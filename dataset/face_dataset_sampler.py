...
maskPath = {'ffceleba': ['/media/xn/SSD1T/ffhq/mask_bin',
            ...
            'illinois': ['/media/xn/SSD1T/illinois/front_mask_bin']}

segPath = {'ffceleba': ['/media/xn/SSD1T/ffhq/mask256_mic_slim',
...
class BaseDataset(Data.Dataset, ABC):
    ...
    def get_data(self, data_id, name):
        ...
        seg_pth = os.path.join(self.seg_root[data_id], coeff_name)
        with open(seg_pth, 'rb') as f:
            seg = pickle.load(f).astype('float32')
        ...
        if self.mode == 'train':
            data = {'img': img, 'uv': uv, 'mat_inv': mat_inverse, 'mask': mask, 'seg':seg}
        ...
        return img, mask, seg, uv, coeff, mat_inverse
...
class FFCelebA(BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        img, mask, seg, uv, coeff, mat_inverse = self.get_data(id, name)
        return img, mask, seg, uv, coeff, mat_inverse
...
class Illinois(BaseDataset):
    ...
    def __getitem__(self, idx):
        name = self.image_list[idx]
        img, mask, seg, uv, coeff, mat_inverse = self.get_data(0, name)
        return img, mask, seg, uv, coeff, mat_inverse
...
if __name__ == '__main__':
    ...
    for idx in range(100):
        img, mask, seg, uv, coeff, mat_inverse = next(fetcher)
        ...
        seg  = make_grid(seg, nrow=8, padding=0)
        seg = seg.permute(1,2,0).detach().cpu().numpy()
        parse_show = vis_parsing_maps(img_show, seg, 1)
