...
class VGG_Activations(nn.Module):
    ...
    def __init__(self, vgg_network, feature_idx, fc=True):
        super(VGG_Activations, self).__init__()
        features = list(vgg_network.features)
        if fc:
            fc_features = [vgg_network.avgpool, torch.nn.Flatten(1)] + list(vgg_network.classifier)
            features = features + fc_features
    ...
    def forward(self, x):
        ...
        for ii, model in enumerate(self.features):
            ...
            if ii in self.idx_list:
                results.append(x)
            if len(results) == len(self.idx_list):
                break
...
def vgg_face_state_dict():
    default = torch.load('../weights/vgg_face_dag.pth')
